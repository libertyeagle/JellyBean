...
use timely::communication::{initialize_from, Allocator, allocator::AllocateBuilder, WorkerGuards};
...
where
    T: Send+'static,
    F: FnOnce(&mut Child<Worker<timely::communication::allocator::thread::Thread>,u64>)->T+Send+Sync+'static
{
    execute_directly(|worker| worker.dataflow(|scope| func(scope)))
}
...
where
    T: Send+'static,
    F: FnOnce(&mut Worker<timely::communication::allocator::thread::Thread>)->T+Send+Sync+'static
{
