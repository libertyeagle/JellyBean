...
use timely::communication::{Allocate, Data, Push, Pull};
use timely::communication::RelayConnectAllocate;
use timely::communication::allocator::thread::{ThreadPusher, ThreadPuller};
use timely::dataflow::channels::MESSAGE_BUFFER_SIZE;
use timely::progress::pipeline_wrapper::PipelineBuilder;
use timely::scheduling::{Schedule, Scheduler, Activations};
use timely::progress::timestamp::{Refines};
use timely::progress::SubgraphBuilder;
use timely::progress::operate::Operate;
use timely::dataflow::scopes::ScopeParent;
use timely::logging::TimelyLogger;
use timely::worker::{AsWorker, Config, RelayConnector, RelayFrontierCommunicationMode};
...
///
/// These methods are often proxied by child scopes, and this trait provides access.try point to a timely dataflow computation. It wraps a `Allocate`,
/// and has a list of dataflows that it manages.
pub struct Worker<A: Allocate + 'static> {
    config: Config,
    timer: Instant,
    ...
    logging: Rc<RefCell<timely::logging_core::Registry<timely::logging::WorkerIdentifier>>>,
...
impl<A: Allocate + 'static> AsWorker for Worker<A> {
    ...
    fn log_register(&self) -> RefMut<timely::logging_core::Registry<timely::logging::WorkerIdentifier>> {
        self.log_register()
    }
}

impl<A: Allocate + 'static> Scheduler for Worker<A> {
    fn activations(&self) -> Rc<RefCell<Activations>> {
        self.activations.clone()
    }
}

impl<A: Allocate + 'static> Worker<A> {
    /// Allocates a new `Worker` bound to a channel allocator.
    pub fn new(config: Config, c: A) -> Worker<A> {
        let now = Instant::now();
        let index = c.index();
        if let Some(buffer_size) = config.get::<usize>("message_buffer_size") {
            MESSAGE_BUFFER_SIZE.set(*buffer_size).unwrap();
            println!("message buffer size set to {}", buffer_size);
        }        
        Worker {
            config,
            timer: now.clone(),
            ...
            logging: Rc::new(RefCell::new(timely::logging_core::Registry::new(now.clone(), index))),
    ...
    pub fn step_or_park(&mut self, duration: Option<Duration>) -> bool {
        ...
        if !self.dataflows.borrow().is_empty() && delay != Some(Duration::new(0,0)) {
            ...
            self.logging().as_mut().map(|l| l.log(timely::logging::ParkEvent::park(delay)));
            ...
            self.logging().as_mut().map(|l| l.log(timely::logging::ParkEvent::unpark()));
        }
    ...
    /// });
    /// ```
    pub fn log_register(&self) -> ::std::cell::RefMut<timely::logging_core::Registry<timely::logging::WorkerIdentifier>> {
        ...
        V: Any+'static,
    {
        ...
        let result = {
            let mut builder: Child<Worker<A>, T> = Child {
                subgraph: subscope.clone(),
                parent: self.clone(),
        ...
        let mut operator = Rc::try_unwrap(subscope).ok().unwrap().into_inner().build(self);

        logging.as_mut().map(|l| l.log(timely::logging::OperatesEvent {
            id: identifier,
...
impl<A: RelayConnectAllocate + 'static> RelayConnector for Worker<A>  {
    fn index(&self) -> usize { self.allocator.borrow().index() }
    fn num_relay_nodes(&self) -> usize { self.allocator.borrow().relay_peers() }
    fn receive_from_relay(&self) {
        let mut allocator = self.allocator.borrow_mut();
        allocator.receive_from_relay();
        allocator.await_relay_events(Some(std::time::Duration::from_millis(1000)));
    }
    fn activate_paths(&self) -> bool  {
        let mut activate_any = false;
        let mut allocator = self.allocator.borrow_mut();
        let relay_events = allocator.relay_events();
        if !relay_events.borrow().is_empty() {
            self.activations.borrow_mut().activate(&[0]);
            relay_events.borrow_mut().clear();
        }
        allocator.receive();
        let events = allocator.events().clone();
        let mut borrow = events.borrow_mut();
    ...
    fn relay_input_pipelines_completed(&self) -> bool {
        self.allocator.borrow().relay_input_pipelines_complete()
    }    
    fn allocate_relay_channel<T: Data>(&mut self, identifier: usize) -> (Vec<Box<dyn Push<Message<T>>>>, Box<dyn Pull<Message<T>>>) {
        self.allocator.borrow_mut().allocate_channel_to_relay(identifier)
    }
}


impl<A: RelayConnectAllocate> Worker<A> {
    pub fn pipeline_step(&mut self) -> bool {
        self.pipeline_step_or_park(Some(Duration::from_secs(0)))
    }

    pub fn pipeline_step_while<F: FnMut()->bool>(&mut self, mut func: F) {
        while func() { self.pipeline_step(); }
    }

    pub fn pipeline_step_or_park_while<F: FnMut()->bool>(&mut self, duration: Option<Duration>, mut func: F) {
        while func() { self.pipeline_step_or_park(duration); }
    }

    pub fn pipeline_step_or_park(&mut self, duration: Option<Duration>) -> bool {
        {
            let mut allocator = self.allocator.borrow_mut();
            allocator.receive_from_relay();
            let relay_events = allocator.relay_events();
    ...
    pub fn pipeline_dataflow<T, R, F>(&mut self, func: F) -> R
    where
        T: Refines<()>,
        F: FnOnce(&mut PipelineScope<A, T>)->R,
    {
        let logging = self.logging.borrow_mut().get("timely");
        self.pipeline_dataflow_core("Dataflow", logging, Box::new(()), |_, child| func(child), Some(RelayFrontierCommunicationMode::MultiChannel))
    }

    pub fn pipeline_dataflow_named<T, R, F>(&mut self, name: &str, func: F) -> R
    where
        T: Refines<()>,
        F: FnOnce(&mut PipelineScope<A, T>)->R,
    {
        let logging = self.logging.borrow_mut().get("timely");
        self.pipeline_dataflow_core(name, logging, Box::new(()), |_, child| func(child), Some(RelayFrontierCommunicationMode::MultiChannel))
    }

    /// Build pipeline dataflow with single channel frontier relay
    pub fn pipeline_dataflow_single_channel_relay<T, R, F>(&mut self, func: F) -> R
    where
        T: Refines<()>,
        F: FnOnce(&mut PipelineScope<A, T>)->R,
    {
        let logging = self.logging.borrow_mut().get("timely");
        self.pipeline_dataflow_core("Dataflow", logging, Box::new(()), |_, child| func(child), Some(RelayFrontierCommunicationMode::SingleChannel))
    }

    /// Build pipeline dataflow with single channel frontier relay
    pub fn pipeline_dataflow_named_single_channel_relay<T, R, F>(&mut self, name: &str, func: F) -> R
        where
            T: Refines<()>,
            F: FnOnce(&mut PipelineScope<A, T>)->R,
    {
        let logging = self.logging.borrow_mut().get("timely");
        self.pipeline_dataflow_core(name, logging, Box::new(()), |_, child| func(child), Some(RelayFrontierCommunicationMode::SingleChannel))
    }


    pub fn pipeline_dataflow_core<T, R, F, V>(
        &mut self,
        name: &str,
        mut logging: Option<TimelyLogger>,
        mut resources: V,
        func: F,
        relay_mode: Option<RelayFrontierCommunicationMode>
    ) -> R
    where
        T: Refines<()>,
        F: FnOnce(&mut V, &mut PipelineScope<A, T>)->R,
        V: Any+'static,
    {
        let addr = vec![];
        let dataflow_index = self.allocate_dataflow_index();
        assert_eq!(dataflow_index, 0, "there can be only one pipeline dataflow");
        let identifier = self.new_identifier();

        let progress_logging = self.logging.borrow_mut().get("timely/progress");
        let relay_progress_logging = self.logging.borrow_mut().get("timely/relay_progress");
        let pipeline = PipelineBuilder::new_from(
            dataflow_index,
            addr,
            logging.clone(),
            progress_logging.clone(),
            relay_progress_logging.clone(),
            name
        );
        let pipeline = Rc::new(RefCell::new(pipeline));

        let result = {
            let mut builder = PipelineScope {
        ...
        let relay_mode = match relay_mode {
            Some(relay_mode) => relay_mode,
            None => RelayFrontierCommunicationMode::MultiChannel
        };

        match relay_mode {
            RelayFrontierCommunicationMode::MultiChannel => {
                let mut operator = Rc::try_unwrap(pipeline).ok().unwrap().into_inner().build_with_multi_channel_relay(
                    self,
                    move |_idx, _cnt| {
            ...
            RelayFrontierCommunicationMode::SingleChannel => {
                let mut operator = Rc::try_unwrap(pipeline).ok().unwrap().into_inner().build_with_single_channel_relay(self);
...
impl<A: Allocate + 'static> ScopeParent for Worker<A> {
    type Timestamp = ();
}

use timely::communication::Message;

impl<A: Allocate + 'static> Clone for Worker<A> {
    fn clone(&self) -> Self {
        Worker {
...
impl Wrapper {
    ...
    fn step(&mut self) -> bool {
        ...
        if let Some(l) = self.logging.as_mut() {
            l.log(timely::logging::ScheduleEvent::start(self.identifier));
        }
        ...
        if let Some(l) = self.logging.as_mut() {
            l.log(timely::logging::ScheduleEvent::stop(self.identifier));
        }
...
impl Drop for Wrapper {
    fn drop(&mut self) {
        if let Some(l) = self.logging.as_mut() {
            l.log(timely::logging::ShutdownEvent { id: self.identifier });
        }
        // ensure drop order
        self.operate = None;
        self.resources = None;
