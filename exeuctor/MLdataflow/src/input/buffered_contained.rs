...
pub struct BufferedContainedInputSource<T: Timestamp, D: Data, D2: Data, L, S: Scope>
where 
...
impl<T: Timestamp + TotalOrder, D: Data, D2: Data, L, S> BufferedContainedInputSource<T, D, D2, L, S>
where
    L: FnMut(D) -> (D2, T) + 'static,
    S: Scope + ScopeParent<Timestamp = T> + 'static
{
    pub fn new(data_stream: VecDeque<D>, emit_logic: L, buffer_size: usize) -> Self {
        BufferedContainedInputSource {
            data_stream,
            buffer: VecDeque::new(),
            buffer_size,
            emit_logic,
    ...
    fn load_buffer(&mut self) {
        ...
        let drain_size = std::cmp::min(self.buffer_size, self.data_stream.len());
        for raw_input in self.data_stream.drain(0..drain_size) {
...
impl<T: Timestamp + TotalOrder, D: Data, D2: Data, L, S> GenericInputFeeder for BufferedContainedInputSource<T, D, D2, L, S>
where
    L: FnMut(D) -> (D2, T) + 'static,
    S: Scope + ScopeParent<Timestamp = T> + 'static
{
...
impl<T, D: Data, D2: ExchangeData, L, A> ExchangeGenericInputFeeder for BufferedContainedInputSource<T, D, D2, L, PipelineScope<A, T>>
where
