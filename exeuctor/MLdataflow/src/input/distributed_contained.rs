...
pub struct WorkerDistributedContainedInputSource<T: Timestamp, D: Data, L, S: Scope>
where 
...
impl<T: Timestamp + TotalOrder, D: Data, L, S> WorkerDistributedContainedInputSource<T, D, L, S>
where
    L: FnMut(&D, &T) -> T + 'static,
    S: Scope + ScopeParent<Timestamp = T> + 'static
{
    pub fn new(data_stream: VecDeque<D>, advance_logic: L, worker_index: usize, worker_peers: usize) -> Self {
        WorkerDistributedContainedInputSource {
            data_stream,
            advance_logic,
            trigger_counter: 0,
            worker_index,
...
impl<T: Timestamp + TotalOrder, D: Data, L, S> GenericInputFeeder for WorkerDistributedContainedInputSource<T, D, L, S>
where
    L: FnMut(&D, &T) -> T + 'static,
    S: Scope + ScopeParent<Timestamp = T> + 'static
{
    ...
    fn step(&mut self) -> bool {
        if self.trigger_counter % self.worker_peers == self.worker_index {
...
impl<T, D: ExchangeData, L, A> ExchangeGenericInputFeeder for WorkerDistributedContainedInputSource<T, D, L, PipelineScope<A, T>>
where
