...
pub struct ContainedInputSource<T: Timestamp, D: Data, L, S: Scope>
where 
...
impl<T: Timestamp + TotalOrder, D: Data, L, S> ContainedInputSource<T, D, L, S>
where
    L: FnMut(&D, &T) -> T + 'static,
    S: Scope + ScopeParent<Timestamp = T> + 'static
{
    pub fn new(data_stream: VecDeque<D>, advance_logic: L) -> Self {
        ContainedInputSource {
            data_stream,
            advance_logic,
...
impl<T: Timestamp + TotalOrder, D: Data, L, S> GenericInputFeeder for ContainedInputSource<T, D, L, S>
where
    L: FnMut(&D, &T) -> T + 'static,
    S: Scope + ScopeParent<Timestamp = T> + 'static
{
...
impl<T, D: ExchangeData, L, A> ExchangeGenericInputFeeder for ContainedInputSource<T, D, L, PipelineScope<A, T>>
where
