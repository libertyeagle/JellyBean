...
use crate::operators_timely::Filter;
...
/// Filter a data stream according to a given logic
pub struct FilterNode<D, L, S> 
where
...
impl<D: Data, L, S> FilterNode<D, L, S>
where
    L: FnMut(&D) -> bool + 'static,
    S: Scope + 'static
{
...
impl<D: Data, L, S> LocalOpBuilder for FilterNode<D, L, S>
where
    L: FnMut(&D) -> bool + 'static,
    S: Scope + 'static
{
    ...
    fn build(&mut self, streams: &[&Box<dyn GenericStream>], config: Option<HashMap<String, Arc<dyn Any + Send + Sync>>>) -> Box<dyn GenericStream> {
        ...
        let stream_intermediate = stream_in.filter(move |x, net_lat| {
        ...
        let stream_out = stream_intermediate.map(move |(x, exec_net_lat)| {
...
impl<D: ExchangeData, L, A, T> ExchangeOpBuilder for FilterNode<D, L, PipelineScope<A, T>>
where
    L: FnMut(&D) -> bool + 'static,
