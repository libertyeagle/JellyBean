...
use crate::node::InspectNode;
use crate::static_timely::timely_static_pipeline_scope::PipelineScope;

use super::Handle;

pub trait Inspect<'a, T, S, D>
where
    T: Timestamp + Refines<()> + TotalOrder,
    S: Scope<Timestamp = T> + 'static,
    D: ExchangeData
{
    fn inspect<L: FnMut(&D) + 'static>(&self, logic: L, name: &str) -> Handle<'a, T, S, D>;
}

pub trait InspectLocal<'a, T, S, D>
where
    T: Timestamp + Refines<()> + TotalOrder,
    S: Scope<Timestamp = T> + 'static,
    D: Data
{
    fn inspect_local<L: FnMut(&D) + 'static>(&self, logic: L, name: &str) -> Handle<'a, T, S, D>;
}

impl<'a, T, A, D> Inspect<'a, T, PipelineScope<A, T>, D> for Handle<'a, T, PipelineScope<A, T>, D> 
where
    T: Timestamp + Refines<()> + TotalOrder,
    A: RelayConnectAllocate + 'static,
    D: ExchangeData
{
    fn inspect<L: FnMut(&D) + 'static>(&self, logic: L, name: &str) -> Handle<'a, T, PipelineScope<A, T>, D> {
        let prev_id = self.id;
        let next_id = self.allocate_new_operator_id();
        let node = InspectNode::<_, _, PipelineScope<A, T>>::new(prev_id, logic);
...
impl<'a, T, S, D> InspectLocal<'a, T, S, D> for Handle<'a, T, S, D> 
where
    T: Timestamp + Refines<()> + TotalOrder,
    S: Scope<Timestamp = T> + 'static,
    D: Data
{
    fn inspect_local<L: FnMut(&D) + 'static>(&self, logic: L, name: &str) -> Handle<'a, T, S, D> {
        let prev_id = self.id;
        let next_id = self.allocate_new_operator_id();
        let node = InspectNode::<_, _, S>::new(prev_id, logic);
