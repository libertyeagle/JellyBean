...
pub trait Abomonation {
    ...
    /// Is the type that implements `Abomonation` a primitive type that does not do anything in
    #[inline(always)] fn is_primitive_type(&self) -> bool { false }
}
...
impl Abomonation for u8 {
    fn is_primitive_type(&self) -> bool { true }
}
...
impl<T: Abomonation> Abomonation for Vec<T> {
    ...
    unsafe fn entomb<W: Write>(&self, write: &mut W) -> IOResult<()> {
        ...
        let inner_entomb_flag = if let Some(elem)= self.first() {
            !elem.is_primitive_type()
        }
        else {
            true
        };
        if inner_entomb_flag {
    ...
    fn extent(&self) -> usize {
        ...
        let inner_extend_flag = if let Some(elem)= self.first() {
            !elem.is_primitive_type()
        }
        else {
            true
        };
        if inner_extend_flag {
