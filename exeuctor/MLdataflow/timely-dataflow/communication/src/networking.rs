...
// my_index is the process index of the current running process
// TCP connections are bidirectional
// we start the connections with process 0..my_index - 1
// and wait the connections from process my_index + 1..(num_processes - 1)
...
pub fn await_connections(addresses: Arc<Vec<String>>, my_index: usize, noisy: bool) -> Result<Vec<Option<TcpStream>>> {
    ...
    for _ in (my_index + 1) .. addresses.len() {
        ...
        // decode contains the reference to the decoded instance, as well as remaining bytes
