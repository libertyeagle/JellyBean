...
{
    // F is a closure to perform the work
    // it takes an allocator
    // it should use the allocator to allocate pushers and pullers
    // perform communication
    // and then return a value (that implments Send and has no non-static references) 
...
pub struct WorkerGuards<T:Send+'static> {
    pub(super) guards: Vec<::std::thread::JoinHandle<T>>,
    pub(super) others: Box<dyn Any+Send>,
}
