// READ, Sep 8 2021
// this package is not really necessary
    ...
    pub fn new_vector(count: usize) -> Vec<ProcessBuilder> {
        ...
        pushers_vec
            ...
            .map(|(index, (pushers, pullers))|
                // len of pushers/pullers: #threads
                ProcessBuilder {
                    index,
                    peers: count,
                    pushers,
...
impl Allocate for ProcessAllocator {
    ...
    fn allocate<T: Data>(&mut self, identifier: usize) -> (Vec<Box<dyn Push<Message<T>>>>, Box<dyn Pull<Message<T>>>) {
        ...
        // Assume and enforce in-order (increasing order) identifier allocation.
    ...
    fn receive(&mut self) {
        ...
        // no inner allocator, receive all data from self.recvs
