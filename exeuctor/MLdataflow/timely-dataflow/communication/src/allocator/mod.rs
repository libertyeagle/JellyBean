...
pub use self::generic::{Generic, GenericBuilder, GenericToRelay, GenericToRelayBuilder};
...
// AllocateBuilder is moved across threads.
...
pub trait Allocate {
    ...
    // returns dyn traits of pushers and puller 
    ...
    // the events should be drained by the user
    ...
    // someone is about to consume the messages and release the resources.
    fn receive(&mut self) { }
    ...
    // make copies of un-consumed buffers to release the scarce resources.
    fn release(&mut self) { }
    ...
    {
        // self.events().clone() clones the Rc pointer to share the data
...
/// Trait for timely workers to allocate channels to relay nodes
pub trait RelayConnectAllocate: Allocate {
    /// Number of nodes in the relay
    fn relay_peers(&self) -> usize;
    /// Allocate channel to relay nodes,
    /// return a pusher to each relay node and a puller
    fn allocate_channel_to_relay<T: Data>(&mut self, channel_identifier: usize) -> (Vec<Box<dyn Push<Message<T>>>>, Box<dyn Pull<Message<T>>>);
    /// Receive messages from the relay node, take in events
    fn receive_from_relay(&mut self) {}
    /// Signal the completion of a batch of reads from channels.
    fn release_relay(&mut self) {}
    /// Communication events to the relay 
    fn relay_events(&self) -> &Rc<RefCell<VecDeque<(usize, Event)>>>;
    /// Whether all inputs (data and frontier updates) have been received input pipelines
    fn relay_input_pipelines_complete(&self) -> bool;
    /// Awaits communication events. from relay
    fn await_relay_events(&self, _duration: Option<Duration>) { }
}
