...
impl<T:Ord> ChangeBatch<T> {
    ...
    pub fn extend<I: Iterator<Item=(T, i64)>>(&mut self, iterator: I) {
        // both the extend and update move the element T
    ...
    pub fn into_inner(mut self) -> Vec<(T, i64)> {
        // consumes `self`
        // makes it compact before return.
        self.compact();
        self.updates
    }
    ...
    pub fn is_empty(&mut self) -> bool {
        // more than (the first) half is compact
        // then even every element after [self.clean..] cancels the compact ones
        // there still msut be remaining updates (elements).
    ...
    pub fn compact(&mut self) {
        if self.clean < self.updates.len() && self.updates.len() > 1 {
            // sort (group) the updates by the element T
            self.updates.sort_by(|x,y| x.0.cmp(&y.0));
            for i in 0 .. self.updates.len() - 1 {
                // merge the consecutive two elements if they are the same element T
