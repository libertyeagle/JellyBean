...
pub trait Timestamp: Clone+Eq+PartialOrder+Debug+Send+Any+Data+Hash+Ord {
    ...
    // the PathSummary type that the Timestamp type corresponds.
...
pub trait PathSummary<T> : Clone+'static+Eq+PartialOrder+Debug+Default {
    ...
    // what happens to a timestamp whenn it passes this path
    ...
    // combine this PathSummary with another PathSumamry
...
macro_rules! implement_timestamp_add {
    // matches a list of type seperated by comma
    ($($index_type:ty,)*) => (
        $(
...
// all of the types implements Copy trait, so the *src, *other above do not move values.
...
mod refines {
    ...
    /// we can't control.
    // Refines is defined on Timestamp types
    pub trait Refines<T: Timestamp> : Timestamp {
        /// Converts the outer timestamp to an inner timestamp.
        
        // T: outer timestamp's type
        // Self: inner timestamp's type
        
        // other is the type of the outer timestamp
        ...
        // inner timestamp goes through the path -> updated inner timestamp
        // and put this timestamp to the outer timestamp
        // this timestamp would be equal to call
        // the returned timestamp from the summarize method on the inner path
        // 's results_in method

        // summarize (packes) the inner path summary to the outer path summary
    ...
    // outer timestamp: unit ()
    // inner timestamp: usize, u128, ...
