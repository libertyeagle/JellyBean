...
impl<T: PartialOrder> Antichain<T> {
    ...
    ///```
    /// NOTE: if T also implements Ord, then Antichain should always have only a single element
    pub fn insert(&mut self, element: T) -> bool {
        // add the element if only none of x (in self.elements) <= element,
        // i.e., for each x, !(x <= element)
        // no x can <= element
        if !self.elements.iter().any(|x| x.less_equal(&element)) {
            // retain only the x that !(element <= x)
            // purge x that element <= x
            // retains the incomparable x that are no greater than element.
    ...
    pub fn less_than(&self, time: &T) -> bool {
        // true if for each x we have x < time
...
impl<T: PartialEq> PartialEq for Antichain<T> {
    fn eq(&self, other: &Self) -> bool {
        ...
        (
            // shortcut, if the positions of the elements are the same
...
impl<T: PartialOrder> PartialOrder for Antichain<T> {
    fn less_equal(&self, other: &Self) -> bool {
        // for every y in other, there must be a x in self that x <= y
...
// exposes the set of elements with positive count not greater than any other elements with positive count.
// Since T implements Ord, frontier should only have one element
...
impl<T: PartialOrder+Ord+Clone> MutableAntichain<T> {
        ...
        I: IntoIterator<Item = (T, i64)>,
    {
        ...
        while self.dirty > 0 && !rebuild_required {
            // look from the updates starting from the first dirty bit
            // (self.updates.len() - self.dirty) is current dirty bit starting index
            // the ones before this bit is clean
            // and self.frontier reflects its Anti frontier
            ...
            // is there any x in frontier that x < time 
            let beyond_frontier = self.frontier.iter().any(|f| f.less_than(time));
            // are all x in frontier satisfy !(x <= time)
            let before_frontier = !self.frontier.iter().any(|f| f.less_equal(time));
            // we don't need to rebuild

            // if there is a x in the frontier that x < time (NOT x <= time), the frontier will not change
            // if we may potentially insert time into the frontier (that is, all x in the frontier have !(x <= time))
            // then, we need to check whether delta < 0 or not. If so, we will also not insert it into the frontier.

            // if there is a x == time, then no matter what (even delta < 0), we need to rebuild the frontier
        ...
        // return changes to the frontier
        // drain() will not consume the collection, but just the elements in it
        // while into_iter() consumes the collection
        self.changes.drain()
    }
    ...
    fn rebuild(&mut self) {
        ...
        // self.changes is used to records the changes to the FRONTIER during rebuild
...
// since it just contains a reference, we have Copy trait
...
// implement IntoIterator for AntichainRef's immutable reference
