...
/// Logger for the timely worker <-> relay progress events
pub type TimelyRelayProgressLogger = Logger<TimelyRelayProgressEvent>;
...
#[derive(Debug)]
/// Send or receive of progress information between the worker and relay nodes.
pub struct TimelyRelayProgressEvent {
    /// `true` if it is sending to the relay node
    /// 'false' if it is receiving from the relay node
    pub is_send: bool,
    /// Source worker (send) or relay node index (receive).
    pub source: usize,
    /// List of message updates, containing Target descriptor, timestamp as string, and delta.
    pub messages: Box<dyn ProgressEventTimestampVec>,
}
...
#[derive(Serialize, Deserialize, Abomonation, Debug, Clone, Hash, Eq, PartialEq, Ord, PartialOrd)]
/// Messages events, send to or received from relay nodes
pub struct RelayMessageEvent {
    /// `true` if send event, `false` if receive event.
    pub is_send: bool,
    /// Scope input / output index
    pub index: usize,
    /// Source worker (send) or relay node index (receive).
    pub source: usize,
    /// Target worker (recv) or relay node index (send)
    pub target: usize,
    /// Number of typed records in this message,
    pub length: usize
}
...
pub enum TimelyEvent {
    ...
    /// Message send to or receive from relay nodes
    RelayMessages(RelayMessageEvent),
    /// Operator start or stop.
...
impl From<RelayMessageEvent> for TimelyEvent {
    fn from(v: RelayMessageEvent) -> TimelyEvent { TimelyEvent::RelayMessages(v) }
}