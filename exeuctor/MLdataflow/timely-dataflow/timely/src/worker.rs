...
use crate::communication::{Allocate, Data, Pull, Push, RelayConnectAllocate};
use crate::communication::allocator::thread::{ThreadPuller, ThreadPusher};
use crate::communication::Message;
use crate::dataflow::channels::MESSAGE_BUFFER_SIZE;
use crate::dataflow::scopes::Child;
use crate::dataflow::scopes::pipeline::PipelineScope;
use crate::logging::TimelyLogger;
use crate::progress::operate::Operate;
use crate::progress::pipeline_wrapper::PipelineBuilder;
...
pub enum RelayFrontierCommunicationMode {
    // Use single channel frontier relay
    SingleChannel,
    MultiChannel
}
...
/// Trait for establishing communication channel to relay nodes
pub trait RelayConnector {
    fn index(&self) -> usize;
    fn num_relay_nodes(&self) -> usize;
...
impl<A: RelayConnectAllocate> RelayConnector for Worker<A> {
    fn index(&self) -> usize { self.allocator.borrow().index() }
...
impl<A: RelayConnectAllocate + 'static> Worker<A> {
        ...
        V: Any+'static,
    {
        ...
        match relay_mode {
            RelayFrontierCommunicationMode::MultiChannel => {
                let mut operator = pipeline.into_inner().build_with_multi_channel_relay(
                    self,
                    move |_idx, _cnt| {
            ...
            RelayFrontierCommunicationMode::SingleChannel => {
                let mut operator = pipeline.into_inner().build_with_single_channel_relay(self);
