...
pub mod link {
    ...
    impl<T, D> EventPusher<T, D> for Rc<EventLink<T, D>> {
        fn push(&mut self, event: Event<T, D>) {
            *self.next.borrow_mut() = Some(Rc::new(EventLink { event: Some(event), next: RefCell::new(None) }));
            ...
            // move self (which is Rc<EventLink<T, D>>) to point to the newly created EventLink at the back
            *self = next;
        }
    }
    ...
    impl<T, D> EventIterator<T, D> for Rc<EventLink<T, D>> {
        fn next(&mut self) -> Option<&Event<T, D>> {
            ...
            if is_some {
                let next = self.next.borrow().as_ref().unwrap().clone();
                // move self (which is Rc<EventLink<T, D>>) to point to the next EventLink
                *self = next;
                self.event.as_ref()
            }
            else {
    ...
    // destroy the list from front to end
    impl<T, D> Drop for EventLink<T, D> {
        fn drop(&mut self) {
            while let Some(link) = self.next.replace(None) {
                // move EventLink out from the shared pointer
...
pub mod binary {
    ...
    impl<T: Abomonation, D: Abomonation, R: ::std::io::Read> EventIterator<T, D> for EventReader<T, D, R> {
        fn next(&mut self) -> Option<&Event<T, D>> {
            ...
            // empty self.consumed elements of buff1, only reserve the portion that starts from self.consumed
            if self.consumed > 0 {
                self.buff2.clear();
            ...
            // read from reader into bytes, if possible
            // push the read bytes into buff1
