...
// also implements the methods for the (immutable and mutable) reference
...
// should be dropped by the USER CODE
// capability is bound to a location (one of the operator's output ports)
pub struct Capability<T: Timestamp> {
    time: T,
    // internal is the system-wide view of all capabilities (of different timestamps and their counts) at this location (operator's output port)
...
impl<T: Timestamp> Capability<T> {
    ...
    // an operator can send messages with timestamp t' only when it holds a capability with t <= t'
    #[inline]
    ...
    pub fn downgrade(&mut self, new_time: &T) {
        ...
        // replace self with the new capability, drops the original capability.
        *self = new_cap;
    }
}
...
pub fn mint<T: Timestamp>(time: T, internal: Rc<RefCell<ChangeBatch<T>>>) -> Capability<T> {
    // inform the system that the current location acquires a new capability with timestamp `time`
...
// inform the system when dropped
...
impl<T: Timestamp> Clone for Capability<T> {
    #[inline]
    // calls mint() to create a clone
    // in order to inform the system
...
impl<T: Timestamp> PartialEq for Capability<T> {
    ...
    fn eq(&self, other: &Self) -> bool {
        // two Capabilities are "equal"
        // only when the time is the same
        // and they are referring to the same location (having the same internal)
...
impl<T: Timestamp> PartialOrder for Capability<T> {
    ...
    fn less_equal(&self, other: &Self) -> bool {
        // self and other are comparable only when they are bounded to the same location
...
impl<T: Timestamp> ::std::hash::Hash for Capability<T> {
    #[inline]
    // hash method takes a Hasher and feeds the data to the hasher
...
impl<'cap, T: Timestamp+'cap> CapabilityTrait<T> for CapabilityRef<'cap, T> {
    ...
    fn valid_for_output(&self, query_buffer: &Rc<RefCell<ChangeBatch<T>>>) -> bool {
        // let borrow = ;
        // if query_buffer refers to one of the output port
        // they we are valid for output at self.time
...
impl<'cap, T: Timestamp+'cap> CapabilityRef<'cap, T> {
    ...
    ///
    /// This method panics if `self.time` is not less or equal to `new_time`.
    // creates a Capability instead of CapabilityRef
    // for output port 0
    #[inline]
    pub fn delayed(&self, new_time: &T) -> Capability<T> {
    ...
    pub fn retain_for_output(self, output_port: usize) -> Capability<T> {
        // create a new Capability for the output_port at timestamp self.time
        if output_port < self.internal.borrow().len() {
...
/// Declared separately so that it can be kept private when `Capability` is re-exported.
#[inline]
//  CapabilityRef is not owned capabilities
// we do not need to inform the system (change the internals)
...
impl<T: Timestamp> ActivateCapability<T> {
    ...
    pub fn downgrade(&mut self, time: &T) {
        ...
        // the previous capability is dropped
        // so we activate the address
...
impl<T: Timestamp> CapabilitySet<T> {
    ...
    pub fn delayed(&self, time: &T) -> Capability<T> {
        // just creates a new capability from the first one in self.elements that can delay to `time`
        ...
        F: IntoIterator<Item=B>,
    {
        ...
        // drop the original capabilities
...
impl<T: Timestamp> Deref for CapabilitySet<T> {
    ...
    // returns a slice of capabilities
