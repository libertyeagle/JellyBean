...
impl<G: Scope> OperatorBuilder<G> {
    ...
    pub fn new_input_connection<D: Data, P>(&mut self, stream: &Stream<G, D>, pact: P, connection: Vec<Antichain<<G::Timestamp as Timestamp>::Summary>>) -> InputHandle<G::Timestamp, D, P::Puller>
        ...
        // when the input port received a new message, it can allocate a new Capability with the timestamp of the message
        // create a new capability out of nowhere
    ...
    // build always indicates the operator is never incomplete
    pub fn build<B, L>(self, constructor: B)
    where
        // the constructor takes a capability for all output ports, and then
        // returns a logic which takes the frontier of the input ports
        ...
        L: FnMut(&[MutableAntichain<G::Timestamp>])->bool+'static
    {
        ...
        // these capabilities we just constructed are dropped here
        // if the inner logic doesn't take the ownership of capabilities
        // these "capabilities" are just passerby that the constructed logic can use them
        // to acquire delayed capabilities to send messages with that timestamp
        // and request notifications at a certain timestamp
        ...
        crate::example(|scope| {
            ...
            builder.build(move |capabilities| {
                move |_frontiers| {
                    ...
                    // the capabilities are bounded to the output port
                    output_handle2.session(&capabilities[0]);
                    output_handle1.session(&capabilities[1]);
                }
                // `logic` is the following closure:
                //                 move |_frontiers| {
                //
                //                     let mut output_handle1 = output1.activate();
                //                     let mut output_handle2 = output2.activate();
