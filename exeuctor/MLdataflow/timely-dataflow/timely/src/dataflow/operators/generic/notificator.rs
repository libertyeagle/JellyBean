...
impl<'a, T: Timestamp> Notificator<'a, T> {
    ...
    // return the frontier of the specified input port
...
impl<T: Timestamp> FrontierNotificator<T> {
    ...
    // with initial pending notifications
    ...
    pub fn notify_at_frontiered<'a>(&mut self, cap: Capability<T>, frontiers: &'a [&'a MutableAntichain<T>]) {
        // if the timestamp to be notified can immediately become available
        // simply insert it into self.available instead of self.pending
...
// a wrapper for Capability that allows us to compare the time in two capabilities
// with reverse time order
#[derive(PartialEq, Eq)]
