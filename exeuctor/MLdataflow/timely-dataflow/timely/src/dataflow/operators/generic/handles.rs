...
// it pulls input data from pull_counter
...
impl<'a, T: Timestamp, D: Data, P: Pull<Bundle<T, D>>> InputHandle<T, D, P> {
    ...
    pub fn next(&mut self) -> Option<(CapabilityRef<T>, RefOrMut<Vec<D>>)> {
        ...
        self.pull_counter.next().map(|bundle| {
            match bundle.as_ref_or_mut() {
                // call mint_capability_ref() to create a capability reference of output ports with capability corresponds to the input messages
    ...
    pub fn for_each<F: FnMut(CapabilityRef<T>, RefOrMut<Vec<D>>)>(&mut self, mut logic: F) {
        ...
        while let Some((cap, data)) = self.pull_counter.next().map(|bundle| {
            ...
            // call logic on each received message with corresponding capability reference
    ...
    #[inline]
    pub fn for_each_with_latency<F: FnMut(CapabilityRef<T>, RefOrMut<Vec<D>>, Option<MessageLatency>)>(&mut self, mut logic: F) {
        // We inline `next()` so that we can use `self.logging` without cloning (and dropping) the logger.
        let internal = &self.internal;
        while let Some((cap, data, lat)) = self.pull_counter.next().map(|bundle| {

            match bundle.as_ref_or_mut() {
                RefOrMut::Ref(bundle) => {
                    let pipeline_latency = bundle.pipeline_latency;
                    (mint_capability_ref(&bundle.time, internal.clone()), RefOrMut::Ref(&bundle.data), pipeline_latency)
                },
                RefOrMut::Mut(bundle) => {
                    let pipeline_latency = bundle.pipeline_latency;
                    (mint_capability_ref(&bundle.time, internal.clone()), RefOrMut::Mut(&mut bundle.data), pipeline_latency)
                },
            }
        }) {
            // call logic on each received message with corresponding capability reference
...
impl<'a, T: Timestamp, D: Data, P: Pull<Bundle<T, D>>+'a> FrontieredInputHandle<'a, T, D, P> {
    ...
    #[inline]
    pub fn for_each_with_latency<F: FnMut(CapabilityRef<T>, RefOrMut<Vec<D>>, Option<MessageLatency>)>(&mut self, logic: F) {
        self.handle.for_each_with_latency(logic)
    }
