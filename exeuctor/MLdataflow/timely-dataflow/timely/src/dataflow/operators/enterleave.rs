...
impl<TOuter: Timestamp, TInner: Timestamp+Refines<TOuter>, TData: Data> Push<Bundle<TOuter, TData>> for IngressNub<TOuter, TInner, TData> {
    ...
    fn push_with_latency_passthrough(&mut self, element: &mut Option<Bundle<TOuter, TData>>, _latency: Option<MessageLatency>) {
        self.push(element);
    }
}
...
where TOuter: Timestamp, TInner: Timestamp+Refines<TOuter>, TData: Data {
    ...
    fn push_with_latency_passthrough(&mut self, element: &mut Option<Bundle<TInner, TData>>, _latency: Option<MessageLatency>) {
        self.push(element);
    }
}
