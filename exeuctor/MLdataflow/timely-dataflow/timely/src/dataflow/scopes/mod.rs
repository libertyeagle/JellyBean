...
// ScopeParent should be able to operate as a worker
// Top-level scope
...
// this trait declares the set of operations that a scope should implement
pub trait Scope: ScopeParent {
    ...
    fn add_operator(&mut self, operator: Box<dyn Operate<Self::Timestamp>>) -> usize {
        ...
        // global: allocates a new worker-unique identifier.
    ...
    // the worker-unique global index is used for something like logging propose, not for progress tracking
    ...
    /// });
    /// ```
    // allocate an iterative child scope
    // the outer scope (parent scope, self)'s timestamp type is <Self as ScopeParent>::Timestamp
    // the child scope's timestamp type is: Product<<Self as ScopeParent>::Timestamp, T>
    // which the inner and outer timestamp can convert to each other via the `Refines` trait
    ...
    /// });
    /// ```
    // the child scope's timestamp does not change
