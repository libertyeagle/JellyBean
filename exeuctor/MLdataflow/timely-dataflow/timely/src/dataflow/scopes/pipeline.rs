...
use crate::communication::{Pull, Push, RelayConnectAllocate};
...
use crate::dataflow::channels::connector::{RelayLogPuller, RelayLogPusher};
use crate::dataflow::channels::pushers::Exchange as ExchangePusher;
use crate::logging::{TimelyLogger as Logger, WorkerIdentifier};
use crate::logging::TimelyProgressLogger as ProgressLogger;
use crate::logging_core::Registry;
use crate::progress::pipeline_wrapper::PipelineBuilder;
use crate::progress::{Operate, Source, SubgraphBuilder, Target, Timestamp};
use crate::progress::timestamp::Refines;
use crate::scheduling::{Activations, Scheduler};
use crate::worker::{AsWorker, Config, RelayConnector, Worker};

/// The root level (pipeline level) scope
/// "wraps" a pipeline builder (which itself is a wrapper of a subgraph builder)
pub struct PipelineScope<'a, A, T>
where
    A: RelayConnectAllocate,
    T: Timestamp+Refines<()>,
{
    /// The pipeline builder
    pub pipeline: &'a RefCell<PipelineBuilder<(), T>>,
...
impl<'a, A, T> PipelineScope<'a, A, T>
where
    A: RelayConnectAllocate,
    T: Timestamp+Refines<()>,
{
    ...
    pub fn register_pipeline_output_balanced_exchange<D: ExchangeData>(&mut self, stream: &Stream<Self, D>, index: usize) {
        let mut counter = 0;
        let num_relays = self.worker.num_relay_nodes() as u64;
        let num_peer_workers = self.worker.peers() as u64;
        let direct_pass_flag = num_peer_workers == num_relays;
        let worker_index = self.worker.index() as u64;
        self.register_pipeline_output::<D, _>(stream, index, move |_| {
            if direct_pass_flag {
                worker_index
            }
            else {
                counter = (counter + 1) % num_relays;
                counter - 1
            }
        });
    }
...
impl<'a, A, T> Scope for PipelineScope<'a, A, T>
where
    A: RelayConnectAllocate,
    T: Timestamp+Refines<()>,
{
...
impl<'a, A, T> ScopeParent for PipelineScope<'a, A, T>
where
    A: RelayConnectAllocate,
    T: Timestamp+Refines<()>,
{
...
impl<'a, A, T> Clone for PipelineScope<'a, A, T>
where
    A: RelayConnectAllocate,
    T: Timestamp+Refines<()>,
{
...
impl<'a, A, T> Scheduler for PipelineScope<'a, A, T>
where
    A: RelayConnectAllocate,
    T: Timestamp+Refines<()>,
{
...
impl<'a, A, T> AsWorker for PipelineScope<'a, A, T>
where
    A: RelayConnectAllocate,
    T: Timestamp+Refines<()>,
{
    ...
    fn allocate<D: crate::communication::Data>(&mut self, identifier: usize, address: &[usize]) -> (Vec<Box<dyn Push<crate::communication::Message<D>>>>, Box<dyn Pull<crate::communication::Message<D>>>) {
        self.worker.allocate(identifier, address)
    }

    fn pipeline<D: 'static>(&mut self, identifier: usize, address: &[usize]) -> (ThreadPusher<crate::communication::Message<D>>, ThreadPuller<crate::communication::Message<D>>) {
