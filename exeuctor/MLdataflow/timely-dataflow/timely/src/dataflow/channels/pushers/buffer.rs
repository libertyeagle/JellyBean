...
impl<T, D, P: Push<Bundle<T, D>>> Buffer<T, D, P> where T: Eq+Clone {
    ...
    pub fn session(&mut self, time: &T) -> Session<T, D, P> {
        // flush if we advance to a different timestamp
    ...
    pub fn cease(&mut self) {
        // signaling a flush, another call to `push()` may not be coming.
        self.flush();
...
impl<'a, T, D, P: Push<Bundle<T, D>>+'a> Session<'a, T, D, P>  where T: Eq+Clone+'a, D: 'a {
    ...
    pub fn give_vec(&mut self, message: &mut Vec<D>) {
        // `message` will be consumed (replaced by an empty vector)
        if message.len() > 0 {
...
// closes session
// flush the buffer, as well as the pusher
