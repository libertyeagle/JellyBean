...
impl<Id: Clone+'static> Registry<Id> {
    ...
    {
        // to_owned create owned data from borrowed one, usually by cloning.
        // when we create a clone of logger, action and buffer are shared
        // if the map alreadys has the key, insert() will return the old value. 
...
pub struct Logger<T, E> {
    // T is the type of the log
    // E is the type of ID
    id:     E,
    // Instant is a measurement of a monotonically nondecreasing clock. Opaque and useful only with Duration.
    // we can take measurement from the clock 
...
impl<T, E: Clone> Clone for Logger<T, E> {
    fn clone(&self) -> Self {
        Logger {
            id: self.id.clone(),
            // Note that Instant implements Copy trait
            time: self.time,
...
impl<T, E: Clone> Logger<T, E> {
    ...
    where
        // 'static bound to trait requires the type does not contain any non-static references
        F: FnMut(&Duration, &mut Vec<(Duration, E, T)>)+'static
    {
        Logger {
            ...
            // the buffer is thread-local shared
            // we can create multiple loggers to the same buffer
            // to ensure that action sees events with increasing timestamps
    ...
    where I: IntoIterator, I::Item: Into<T>
    {
        ...
        for event in events {
            // .into(): a value-to-value conversion that consumes the input value.
            ...
            if buffer.len() == buffer.capacity() {
                ...
                // we must have a mutable reference to a FnMut closure in order to call it
                // through the mutable reference, action must take the events in buffer
...
// i.e., we only have to flush when the last remaining Logger instance that uses this buffer drops
