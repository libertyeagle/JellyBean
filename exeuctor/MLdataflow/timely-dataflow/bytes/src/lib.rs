...
pub mod arc {
    ...
    pub struct Bytes {
        ...
        // this is shared among multiple threads
        // we should not mutate it after we allocate the space in the `from` method
    ...
    impl Bytes {
        ...
        pub fn from<B>(bytes: B) -> Bytes where B : DerefMut<Target=[u8]>+'static {
            ...
            // B is a data type that contains bytes, it can be dereferenced to mutable reference to u8 slice (&mut [u8])
            // e.g., Vec<u8>
            let mut sequestered = Arc::new(bytes) as Arc<dyn Any>;
            let (ptr, len) =
            // get_mut() returns a mutable reference into the given Arc, if there are no other Arc or Weak pointers to the same allocation.
        ...
        pub fn try_regenerate<B>(&mut self) -> bool where B: DerefMut<Target=[u8]>+'static {
            ...
            // get_mut will check that current Arc has unique access to it
        ...
        pub fn try_merge(&mut self, other: Bytes) -> Result<(), Bytes> {
            // self and other need to point to the same underlying resources
